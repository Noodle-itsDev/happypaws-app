...
const ImageCarousel: React.FC<ImageCarouselProps> = ({ items }) => {
  ...
  useEffect(() => {
    if (items.length > 0 && textRef.current) {
      gsap.fromTo(
        textRef.current,
        { opacity: 0, y: '100%' },
        { duration: 0.6, opacity: showText ? 1 : 0, y: showText ? '0%' : '100%', ease: 'power2.out' }
      );
    }
  }, [showText, items.length]);

  useEffect(() => {
    if (items.length > 0 && imageRef.current) {
      gsap.fromTo(
        imageRef.current,
